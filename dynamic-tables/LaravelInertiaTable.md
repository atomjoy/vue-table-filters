...
class TodoController extends Controller
{
    ...
    {
        // Rozpoczynamy budowanie zapytania Eloquent
        $query = Todo::query();

        // 1. SZUKANIE (Server-side)
        ...
        // 2. FILTROWANIE PO STATUSIE Checkbox
        ...
        // 3. SORTOWANIE (Zabezpieczone białą listą kolumn)
        ...
        // 4. PAGINACJA (Laravel automatycznie dołączy parametry query string do linków)
...
// Przechwytujemy dane przesłane z Laravela jako props
...
// JEDEN obiekt stanu aplikacji, zsynchronizowany z tym, co przysłał serwer
...
// Reagujemy na zmianę statusu lub limitu stron – natychmiast odświeżamy tabelę
watch(
  () => [
  ...
  () => {
    // 1. Czyścimy poprzedni timer, jeśli użytkownik zmienił filtr przed upływem 300ms
    clearTimeout(debouncerId)
    // 2. Uruchamiamy licznik, który wyśle zapytanie dopiero gdy użytkownik przestanie klikać
    debouncerid = setTimeout(() => {
      updateTable()
    }, 300) // 300ms to optymalny czas reakcji
  },
)
// ref
...
// Obsługa wyszukiwania (tutaj przydałby się prosty debounce, żeby nie strzelać serwerem co literkę)
function onSearch() {
  updateTable()
}
